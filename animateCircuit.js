...
const pathLength = track.getTotalLength();
    ...
    progress.setAttribute("stroke-dasharray", pathLength);
    progress.setAttribute("stroke-dashoffset", pathLength);
  // Longueur de scroll contrôlée dynamiquement
  const stickyHeight = window.innerHeight * 7; // 300vh de scroll
const rect = stickySection.getBoundingClientRect()
console.log(rect.height)
ScrollTrigger.create({
  trigger: ".Circuit_monaco",
  start: "top top",
  end: `+=${stickyHeight}px`,
  scrub: true,
  pin: true,
  ...
  onUpdate: (self) => {
    ...
    const progressRatio = 1 - self.progress;
    const offset = self.progress * pathLength;
    progress.setAttribute("stroke-dashoffset", pathLength - offset);
    handleCheckpoints(progressRatio);
    
}
});

  function handleCheckpoints(progress) {
    // Pour 16 checkpoints, on fixe des thresholds espacés
  const steps = [
    { id: "#step-1", threshold: 0.85 },
    { id: "#step-2", threshold: 0.76 },
    { id: "#step-3", threshold: 0.69 },
    { id: "#step-4", threshold: 0.65 },
    { id: "#step-5", threshold: 0.58 },
    { id: "#step-6", threshold: 0.546 },
    { id: "#step-7", threshold: 0.52 },
    { id: "#step-8", threshold: 0.42 },
    { id: "#step-9", threshold: 0.30 },
    { id: "#step-10", threshold: 0.27 },
    { id: "#step-11", threshold: 0.214 },
    { id: "#step-12", threshold: 0.172 },
    { id: "#step-13", threshold: 0.14 },
    { id: "#step-14", threshold: 0.099 },
    { id: "#step-15", threshold: 0.06 },
    { id: "#step-16", threshold: 0.01 },
  ];
 // Trouve le premier checkpoint qui correspond à la progression
  let currentStep = null;
  let minDiff = Infinity;

  steps.forEach(step => {
    const diff = progress - step.threshold;
